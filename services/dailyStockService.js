...
class DailyStockService {
  ...
  static async generateWeeklySummary(startDate, endDate) {
    try {
      // Get all daily stock records for the date range
      ...
      // Group by medication and service user
      const groupedData = {};
      for (const stock of dailyStocks) {
        const key = `${stock.medication._id}-${stock.serviceUser._id}`;
        if (!groupedData[key]) {
          groupedData[key] = {
            medication: stock.medication,
            serviceUser: stock.serviceUser,
            changes: [],
            cumulativeChanges: {
              fromPharmacy: 0,
              quantityAdministered: 0,
        ...
        // Add changes from daily stock
        for (const change of stock.changes) {
          const user = await User.findById(change.updatedBy).select(
            "username email"
          );
          groupedData[key].changes.push({
            type: change.type,
            quantity: change.quantity,
            note: change.note,
            timestamp: change.timestamp,
            updatedBy: {
              _id: change.updatedBy,
          ...
          const changeType = change.type.toLowerCase().replace(/\s+/g, "");
          if (groupedData[key].cumulativeChanges.hasOwnProperty(changeType)) {
            groupedData[key].cumulativeChanges[changeType] += change.quantity;
          }
        }
      }

      // Create summary records
      const summaries = Object.values(groupedData).map((data) => ({
        serviceUser: {
          _id: data.serviceUser._id,
          name: data.serviceUser.name,
          dateOfBirth: data.serviceUser.dateOfBirth,
          nhsNumber: data.serviceUser.nhsNumber,
        },
        medication: {
          _id: data.medication._id,
          medicationName: data.medication.medicationName,
          quantityInStock: data.medication.quantityInStock,
          quantityPerDose: data.medication.quantityPerDose,
          dosesPerDay: data.medication.dosesPerDay,
        },
        stockLevels: {
          initial: data.changes[0]?.quantity || 0,
          final: data.medication.quantityInStock,
          daysRemaining: Math.floor(
            data.medication.quantityInStock /
              (data.medication.quantityPerDose * data.medication.dosesPerDay)
          ),
        },
        cumulativeChanges: data.cumulativeChanges,
  ...
  static async getSummaryByDateRange(startDate, endDate) {
    try {
      ...
      if (!summary) {
        summary = await this.generateWeeklySummary(startDate, endDate);
      }
